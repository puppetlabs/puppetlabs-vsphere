...
test_name 'CLOUD-372 - C68521 - vcenter.conf file has wrong VSPHERE_USER'
test_name 'CLOUD-372 - C68529 - vcenter.conf file missing VSPHERE_USER'
...
#test 1: creating a VM with wrong VCENTER_USER
...
#test 2: creating a VM with missing VCENTER_USER
step "Delete user param in vcenter.conf "
vcenter_conf_erb = vcenter_conf_erb.sub(/user.*/,"")
...
confine_block :except, :roles => %w{master dashboard database} do
  agents.each do |agent|
    ...
    on(agent, puppet('agent', '-t', '--environment production'), :acceptable_exit_codes => 1) do |result|
      ...
      assert_match(/To use this module you must provide the following settings: user/, result.output, 'Test Failed')
    end
  end
end
