...
confine_block :except, :roles => %w{master dashboard database} do
  ...
  agents.each do |agent|
    ...
    step "puppet source vsphere_vm original: '#{name}':"
    on(agent, puppet('resource', 'vsphere_vm', name)) do |result|
      assert_match(/cpu.*#{cpu}/, result.output, 'Failed to create specified CPU')
...
memory       = '2048'
cpu          = '2'
annotation   = 'VM with 2 CPU and 2GB RAM'
...
confine_block :except, :roles => %w{master dashboard database} do
  ...
  step "puppet source vsphere_vm mofified: '#{name}':"
  on(agent, puppet('resource', 'vsphere_vm', name)) do |result|
    assert_match(/cpu.*#{cpu}/, result.output, 'Failed to create specified CPU')
    assert_match(/memory.*#{memory}/, result.output, 'Failed to create specified memory')
    puts result
