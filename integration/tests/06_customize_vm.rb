...
confine_block :except, roles: ['master', 'dashboard', 'database'] do
  ...
  on(agent, puppet('resource', 'vsphere_vm', "#{folder}/#{name}")) do |result|
    assert_match(%r{cpu.*#{cpus}}, result.output, 'Failed to create specified CPU')
    assert_match(%r{memory.*#{memory}}, result.output, 'Failed to create specified memory')
  end
end
...
confine_block :except, roles: ['master', 'dashboard', 'database'] do
  on(agent, puppet('agent', '-t', '--environment production'), acceptable_exit_codes: [0, 2]) do |result|
    assert_match(%r{memory: memory changed '512' to '2048'}, result.output, "Failed to customize the VM: '#{name}'")
    assert_match(%r{cpus: cpus changed '1' to '2'}, result.output, "Failed to customize the VM: '#{name}'")
    assert_match(%r{annotation: annotation changed.*VM with 2 CPUs and 2GB RAM}, result.output, "Failed to customize the VM: '#{name}'")
  end
end
