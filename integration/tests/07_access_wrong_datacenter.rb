...
confine_block :except, roles: ['master', 'dashboard', 'database'] do
  ...
  on(agent, puppet('resource', 'vsphere_vm', "non-exist-datacenter/vm/eng/integration/vm/#{name}")) do |result|
    assert_match(%r{ensure.*absent}, result.output, 'Failed to look for machine info with wrong datacenter')
    assert_no_match(%r{Error}, result.output, 'Failed to look for machine info with wrong datacenter')
  end
end
