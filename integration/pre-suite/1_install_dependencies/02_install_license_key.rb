...
step 'Create /etc/puppetlabs/license.key file...' do
  create_remote_file(master, '/etc/puppetlabs/license.key', license_content)
...
step 'Validate the /etc/puppetlabs/license.key' do
  on(master, puppet('license'), acceptable_exit_code: [0]) do |result|
    assert_match(%r{You are currently licensed for}, result.stdout, "'puppet license' test failed!!!")
  end
end
